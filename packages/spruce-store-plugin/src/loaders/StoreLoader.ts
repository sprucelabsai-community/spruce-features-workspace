...
export class StoreLoader {
	...
	private constructor(storesDir: string, db: Database) {
		this.storesDir = storesDir
		this.db = db
	}

	public static async Loader(storesDir: string, db: Database) {
		return new this(storesDir, db)
	}
	...
	private async loadStoreClassesWithErrors(): Promise<{
		...
		const matches = await globby(`${this.storesDir}/**/*.store.[j|t]s`)
